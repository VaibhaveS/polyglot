...
use polyglot_sql::{parse, transpile, Dialect, DialectType, TranspileOptions};
...
#[test]
fn postgres_to_number_grouping_and_space_masks_do_not_lower_to_try_convert_for_fabric() {
    let cases = [
    ...
    for sql in cases {
        ...
        let best_effort = pg_to_fabric(sql);
        assert!(
...
fn postgres_parenthesized_lateral_joins_map_to_fabric_apply() {
    ...
    for (sql, expected) in cases {
        for generated in [pg_to_fabric(sql), pg_to_fabric_strict(sql)] {
            assert_eq!(generated, expected, "failed for {sql}");
            polyglot_sql::parse(&generated, DialectType::Fabric)
...
#[test]
fn postgres_row_value_equality_issue_271_examples_preserve_three_valued_semantics_for_fabric() {
    let cases = [
        (
...
#[test]
fn postgres_row_value_equality_preserves_zero_and_multiple_row_cardinality_for_fabric() {
    let zero_rows = pg_to_fabric_strict(
    ...
    let potentially_multiple_rows =
        pg_to_fabric_strict("SELECT ROW(1, 2) = (SELECT f1, f2 FROM subselect_tbl) AS eq");
    assert!(
...
#[test]
fn postgres_row_value_equality_handles_nulls_aliases_and_query_modifiers_for_fabric() {
    let null_component = pg_to_fabric_strict("SELECT ROW(1, NULL) = (SELECT 1, 2) AS eq");
    assert!(
    ...
    let alias_collision = pg_to_fabric_strict(
    ...
    let query_modifiers = pg_to_fabric_strict(
        "SELECT ROW(1, 2) = \
...
#[test]
fn issue_382_postgres_scalar_array_quantifiers_lower_to_fabric_predicates() {
    let cases = [
        (
    ...
    for (sql, expected) in cases {
        assert_eq!(pg_to_fabric(sql), expected, "default failed for {sql}");
        assert_eq!(
            pg_to_fabric_strict(sql),
            expected,
            "strict failed for {sql}"
        );
        parse(expected, DialectType::Fabric).unwrap_or_else(|error| {
            panic!("generated Fabric failed to parse for {sql:?}: {error}")
        });
    }
}
